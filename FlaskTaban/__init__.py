...
taban = KekikTaban(
    baslik   = "@keyiflerolsun FlaskTaban",
    aciklama = "FlaskTaban Başlatıldı..",
    banner   = "FlaskTaban",
    girinti  = 4
)
...
AYAR     = load(open("ayar.json", "r", encoding="utf-8"))
MONGO_DB = AYAR['MongoDB']

if MONGO_DB:
    from Lib.DB._MongoDB import FlaskTabanDB
else:
    from Lib.DB._TinyDB import FlaskTabanDB
...
####
from FlaskTaban._endpointler import _hata, ana_sayfa, giris_yap, kayit_ol, monitor
